...
CREATE TABLE IF NOT EXISTS game_batting (
    ...
    flyOuts INTEGER CHECK (flyOuts >= 0),
    groundOuts INTEGER CHECK (groundOuts >= 0),
    ...
    baseOnBalls INTEGER CHECK (baseOnBalls >= 0),
    intentionalWalks INTEGER CHECK (intentionalWalks >= 0),
    hits INTEGER CHECK (hits >= 0),
    hitByPitch INTEGER CHECK (hitByPitch >= 0),
    atBats INTEGER CHECK (atBats >= 0),
    caughtStealing INTEGER CHECK (caughtStealing >= 0),
    stolenBases INTEGER CHECK (stolenBases >= 0),
    groundIntoDoublePlay INTEGER CHECK (groundIntoDoublePlay >= 0),
    groundIntoTriplePlay INTEGER CHECK (groundIntoTriplePlay >= 0),
    plateAppearances INTEGER CHECK (plateAppearances >= 0),
    totalBases INTEGER CHECK (totalBases >= 0),
    rbi INTEGER CHECK (rbi >= 0),
    leftOnBase INTEGER CHECK (leftOnBase >= 0),
    sacBunts INTEGER CHECK (sacBunts >= 0),
    sacFlies INTEGER CHECK (sacFlies >= 0),
    catchersInterference INTEGER CHECK (catchersInterference >= 0),
    pickoffs INTEGER CHECK (pickoffs >= 0),
...
CREATE TABLE IF NOT EXISTS game_pitching (
    ...
    gamesPlayed INTEGER CHECK (gamesPlayed >= 0),
    gamesStarted INTEGER CHECK (gamesStarted >= 0),
    flyOuts INTEGER CHECK (flyOuts >= 0),
    groundOuts INTEGER CHECK (groundOuts >= 0),
    airOuts INTEGER CHECK (airOuts >= 0),
    runs INTEGER CHECK (runs >= 0),
    doubles INTEGER CHECK (doubles >= 0),
    triples INTEGER CHECK (triples >= 0),
    homeRuns INTEGER CHECK (homeRuns >= 0),
    ...
    caughtStealing INTEGER CHECK (caughtStealing >= 0),
    stolenBases INTEGER CHECK (stolenBases >= 0),
    numberOfPitches INTEGER CHECK (numberOfPitches >= 0),
    ...
    saveOpportunities INTEGER CHECK (saveOpportunities >= 0),
    holds INTEGER CHECK (holds >= 0),
    blownSaves INTEGER CHECK (blownSaves >= 0),
    earnedRuns INTEGER CHECK (earnedRuns >= 0),
    battersFaced INTEGER CHECK (battersFaced >= 0),
    outs INTEGER CHECK (outs >= 0),
    gamesPitched INTEGER CHECK (gamesPitched >= 0),
    completeGames INTEGER CHECK (completeGames >= 0),
    shutouts INTEGER CHECK (shutouts >= 0),
    pitchesThrown INTEGER CHECK (pitchesThrown >= 0),
    balls INTEGER CHECK (balls >= 0),
    strikes INTEGER CHECK (strikes >= 0),
    strikePercentage TEXT,
    hitBatsmen INTEGER CHECK (hitBatsmen >= 0),
    balks INTEGER CHECK (balks >= 0),
    wildPitches INTEGER CHECK (wildPitches >= 0),
    pickoffs INTEGER CHECK (pickoffs >= 0),
    rbi INTEGER CHECK (rbi >= 0),
    gamesFinished INTEGER CHECK (gamesFinished >= 0),
    runsScoredPer9 TEXT,
    homeRunsPer9 TEXT,
    inheritedRunners INTEGER CHECK (inheritedRunners >= 0),
    inheritedRunnersScored INTEGER CHECK (inheritedRunnersScored >= 0),
    catchersInterference INTEGER CHECK (catchersInterference >= 0),
    sacBunts INTEGER CHECK (sacBunts >= 0),
    sacFlies INTEGER CHECK (sacFlies >= 0),
    passedBall INTEGER CHECK (passedBall >= 0),
